  ...
  if (cart.length === 0) {
    return (
      <div className="h-full flex flex-col items-center justify-center p-12 text-center space-y-8 bg-black">
        <div className="w-32 h-32 bg-white/[0.02] border border-white/5 rounded-full flex items-center justify-center text-[#FF4D00]/20 animate-pulse">
          <ShoppingBag className="w-12 h-12" />
        </div>
        <div className="space-y-4">
          <h3 className="text-2xl font-black uppercase text-white tracking-tighter">Archive Empty</h3>
          <p className="hud-text text-white/30 italic">No artifacts selected for preservation.</p>
        </div>
        <SheetClose asChild>
          <button className="py-4 px-12 border border-[#FF4D00] text-[#FF4D00] text-[10px] font-black uppercase tracking-[0.4em] hover:bg-[#FF4D00] hover:text-white transition-all">
            Return_To_Vault
          </button>
        </SheetClose>
      </div>
    ...
    <div className="h-full flex flex-col bg-black">
      <SheetHeader className="p-8 border-b border-white/5">
        <div className="flex items-center justify-between">
          ...
          <button onClick={clearCart} className="hud-text text-red-500/40 hover:text-red-500 transition-colors">
            Purge_All
          </button>
        </div>
      </SheetHeader>
          ...
          {cart.map((item: CartItem) => (
            <div key={item.id} className="flex gap-8 group">
              ...
              <div className="flex-grow space-y-4">
                ...
                <div className="flex items-center justify-between">
                   <p className="text-[#FF4D00] font-mono font-bold tracking-tighter text-lg">₹{item.price}</p>
                   <div className="flex items-center gap-4">
                      <button 
                        onClick={() => updateQuantity(item.id, -1)}
                        className="w-8 h-8 border border-white/10 flex items-center justify-center text-white/20 hover:border-[#FF4D00] hover:text-white transition-all"
                      >
      ...
      <div className="p-8 border-t border-white/5 space-y-8">
        <div className="space-y-3">
          ...
          <div className="flex justify-between items-end">
            ...
            <span className="text-3xl font-mono font-black text-[#FF4D00] tracking-tighter">₹{grandTotal.toFixed(2)}</span>
          </div>
        </div>
        <button className="w-full h-20 bg-[#FF4D00] text-white text-[12px] font-black uppercase tracking-[0.5em] shadow-[0_0_40px_rgba(255,77,0,0.3)] hover:scale-[1.02] active:scale-95 transition-all">
          Execute_Payment
        </button>
      </div>
    </div>
